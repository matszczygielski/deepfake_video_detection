...
@click.option("--num-convlstm-filters", type=click.INT, default=256, help="Number of convLSTM filters in model.")
@click.option("--convlstm-kernel-size", type=click.INT, default=5, help="Kernel size of convLSTM filters.")
...
@click.option("--out-model-path", type=click.STRING, default="../models/cnnconvlstm.h5", help="Path of output model.")
@click.argument("dataset_path", type=click.STRING)
def train(dataset_path,
          in_width, in_height, sequence_len,
          cnn_model, do_freeze_backbone,
          num_convlstm_filters, convlstm_kernel_size, num_dense_units,
    ...
    def create_model():
        ...
        x = layers.ConvLSTM2D(filters=num_convlstm_filters,
                              kernel_size=(convlstm_kernel_size, convlstm_kernel_size))(x)
        x = layers.Flatten()(x)
